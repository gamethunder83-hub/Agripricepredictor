  ...
  async function handleTraining() {
    ...
    try {
      const response = await trainModel({
        commodity,
        model_type: modelType === "lstm" ? "both" : modelType
      });
  ...
  return (
    <div className="page-shell">
      <header className="hero-panel">
        ...
        <div className="hero-card">
          <div className="hero-card-row">
            <Leaf size={18} />
            <span>Commodity signals + seasonality</span>
          </div>
          <div className="hero-card-row">
            <CloudSun size={18} />
            <span>Weather-aware forecasting input</span>
          </div>
          <div className="hero-card-row">
            <Cpu size={18} />
            <span>Trainable Random Forest and LSTM models</span>
          </div>
          <div className="hero-card-row">
            <TrendingUp size={18} />
            <span>Confidence interval for decision support</span>
          </div>
        </div>
      </header>
      ...
      <section className="control-panel">
        ...
        <button className="primary-button" onClick={() => loadDashboardData()}>
          <RefreshCw size={16} />
          Refresh forecast
        </button>
        <button className="secondary-button" onClick={handleTraining} disabled={training}>
      ...
      <section className="summary-grid">
        <SummaryCard
          ...
          value={latestHistoryPoint ? `Rs. ${latestHistoryPoint.modal_price}` : "--"}
          hint="Most recent market modal price"
        />
        <SummaryCard
          label="Forecast end price"
          value={lastPrediction ? `Rs. ${lastPrediction.predicted_price}` : "--"}
          hint={`${horizonDays}-day forecasted terminal price`}
          accent="grain"
        />
        <SummaryCard
          label="Expected movement"
          value={trendDelta}
          hint="Relative change from latest observed price"
          accent="soil"
        />
        <SummaryCard
          label="Model accuracy"
          value={predictionPayload.metrics?.rmse ? `RMSE ${predictionPayload.metrics.rmse}` : "Pending"}
          hint="Latest saved training metric"
          accent="sky"
        />
      </section>
      ...
      <section className="chart-grid">
        <article className="panel">
          <div className="panel-header">
            <div>
              <span className="panel-kicker">Historical market trend</span>
              <h2>Past price movement</h2>
            </div>
            <span className="panel-chip">{loading ? "Loading" : "Updated"}</span>
          </div>
        ...
        <article className="panel warm">
          <div className="panel-header">
            <div>
              <span className="panel-kicker">Projected trajectory</span>
              <h2>Future price forecast</h2>
            </div>
            <span className="panel-chip">{modelType.replace("_", " ")}</span>
          </div>
      ...
      <section className="lower-grid">
        <article className="panel">
          <div className="panel-header">
            <div>
              <span className="panel-kicker">Prediction table</span>
              <h2>Day-wise forecast output</h2>
            </div>
          </div>
        ...
        <article className="panel notes-panel">
          <div className="panel-header">
            <div>
              <span className="panel-kicker">System insight</span>
          ...
          <div className="metric-list">
            <div>
              <span>MAE</span>
              <strong>{predictionPayload.metrics?.mae || "--"}</strong>
            </div>
            <div>
              <span>RMSE</span>
              <strong>{predictionPayload.metrics?.rmse || "--"}</strong>
            </div>
            <div>
              <span>R2</span>
              <strong>{predictionPayload.metrics?.r2 || "--"}</strong>
            </div>
          </div>
